...
def load_opt_weights(model_name, path):
    ...
    local_folder = "/home/tangm_lab/cse12112106/weights" + model_name
    bin_files = glob.glob(os.path.join(local_folder, "*.bin"))
    
    if not bin_files:
        raise FileNotFoundError("No .bin files found in the specified local folder.")
    ...
    for bin_file in tqdm(bin_files, desc="Convert format"):
        ...
        for name, param in tqdm(state.items(), leave=False):
            ...
            if "decoder.embed_tokens.weight" in name:
                shutil.copy(param_path, param_path.replace("decoder.embed_tokens.weight", "lm_head.weight"))
